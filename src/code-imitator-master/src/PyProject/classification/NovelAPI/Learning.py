...
class Learning:
    ...
    def do_learn(self, train_obj: StyloFeatures,
        ...
        print(">Whole train set: TRAIN:", train_obj.getfeaturematrix().shape[0], "TEST:", test_obj.getfeaturematrix().shape[0], file=sys.stderr)
    ...
    def learn_with_cv(self, train_obj: StyloFeatures,
        ...
        if learn_method == "RF":
            acc, clf = self.random_forest_learning(x_train=x_train, y_train=y_train, x_test=x_test, y_test=y_test,
                                              listoftraintestsplits=listoftraintestsplits,
                                              config_learning=config_learning, problemid_test=problemid_test)
    ...
    def random_forest_learning(self, x_train, y_train, x_test, y_test, listoftraintestsplits, config_learning, problemid_test):
        ...
        if problemid_test == "3264486_5633382285312000":
            challenage = "./random_forest_3264486_5633382285312000.joblib"
            prob = "./prob"+pn+"/prob1/prob_"

        elif problemid_test == "3264486_5654742835396608":
            challenage = "./random_forest_3264486_5654742835396608.joblib"
            prob = "./prob"+pn+"/prob2/prob_"

        elif problemid_test == "3264486_5736519012712448":
            challenage = "./random_forest_3264486_5736519012712448.joblib"
            prob = "./prob"+pn+"/prob3/prob_"

        elif problemid_test == "5304486_5697460110360576":
            challenage = "./random_forest_5304486_5697460110360576.joblib"
            prob = "./prob"+pn+"/prob4/prob_"

        elif problemid_test == "5304486_5760761888505856":
            challenage = "./random_forest_5304486_5760761888505856.joblib"
            prob = "./prob"+pn+"/prob5/prob_"

        elif problemid_test == "8294486_5630967708385280":
            challenage = "./random_forest_8294486_5630967708385280.joblib"
            prob = "./prob"+pn+"/prob6/prob_"

        elif problemid_test == "8294486_5654117850546176":
            challenage = "./random_forest_8294486_5654117850546176.joblib"
            prob = "./prob"+pn+"/prob7/prob_"

        elif problemid_test == "8294486_5681755159789568":
            challenage = "./random_forest_8294486_5681755159789568.joblib"
            prob = "./prob"+pn+"/prob8/prob_"


        print(challenage, file=sys.stderr)
        print(prob, file=sys.stderr)



        # best_params, best_params_acc = self.customized_grid_search(param_grid=param_grid,clf=RandomForestClassifier(random_state=31, n_jobs=config_learning.noofparallelthreads), listoftraintestsplits=listoftraintestsplits)
        #
        ...
        # joblib.dump(rlf_best, challenage)
        # # #
        rlf_best = joblib.load(challenage)
        yprob = rlf_best.predict_proba(x_test)

        print("proba", file=sys.stderr)
        print(yprob, file=sys.stderr)

        result = np.array2string(yprob, precision=2, separator=',',    suppress_small=True)
        for x in range(int(yprob.shape[0])):

            filename = prob+str(x)+".txt"
            with open(filename, 'w') as f:
                for y in range(int(yprob.shape[0])):
                    f.writelines(np.array2string(yprob[x][y], precision=5, separator=',',    suppress_small=True))
                    f.writelines("\n")
            # elif (x == 87):
            #     filename = prob+str(x)+".txt"
            #     with open(filename, 'w') as f:
        ...
        print("RF-Acc:", file=sys.stderr)
        print(theaccuracy, file=sys.stderr)

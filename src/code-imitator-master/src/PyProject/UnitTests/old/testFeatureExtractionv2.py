...
class TestFeatureExtraction(unittest.TestCase):
    ...
    def setUp(self):
        ...
        # self.assertAlmostEqual(float(features_merged.codestyloreference.codestyloreference.getfeaturematrix().mean()),
        #                        0.4633481502532959, delta=1e-05) # float cmd just for type hints
    ...
    def __settrain(self):
        ...
        self.assertAlmostEqual(minmaxvals[1], 27.0, delta=1e-07)

        #self.assertAlmostEqual(self.data_final_train.getfeaturematrix().mean(), 0.019612662, delta=10e-6)
        self.assertAlmostEqual(self.data_final_train.getfeaturematrix().mean(), 0.099762231, delta=1e-05)
        ...
        self.assertEqual(hashlib.md5(xnms_str.encode()).hexdigest(), '573b625327fec5301d7eb759faa1f263')
    ...
    def __settest(self):
        ...
        self.assertAlmostEqual(minmaxvals[1], 20.0, delta=1e-07)

        #self.assertAlmostEqual(self.data_final_test.getfeaturematrix().mean(), 0.0159195, delta=10e-6)
        self.assertAlmostEqual(self.data_final_test.getfeaturematrix().mean(), 0.080948181, delta=1e-05)
    ...
    def __settraintestlexems(self):
        ...
        self.assertAlmostEqual(self.data_final_train_lexems.getfeaturematrix().mean(), 0.48251969, delta=1e-05)

        #self.assertAlmostEqual(self.data_final_test_lexems.getfeaturematrix().mean(), 0.060499839, delta=10e-6)
        self.assertAlmostEqual(self.data_final_test_lexems.getfeaturematrix().mean(), 0.30663744, delta=1e-05)

        #self.assertAlmostEqual(sum_lex_test, 360545.88, delta=10e-1)
        self.assertAlmostEqual(sum_lex_test, 34379.879, delta=1.0)

        # self.assertAlmostEqual(self.data_final_test_lexems.codestyloreference._featurematrix.mean(), 0.061677273)
        self.assertAlmostEqual(self.data_final_test_lexems.codestyloreference._featurematrix.mean(), 0.28462669)
    ...
    def __settraintestlexicalsarff(self):
        ...
        self.assertAlmostEqual(self.data_final_train_lexicals.getfeaturematrix().mean(), 0.43472695, delta=1e-05)

        #self.assertAlmostEqual(self.data_final_test_lexicals.getfeaturematrix().mean(), 0.055086121, delta=10e-6)
        self.assertAlmostEqual(self.data_final_test_lexicals.getfeaturematrix().mean(), 0.28077498, delta=1e-05)
        ...
        self.assertEqual(hashlib.md5(xnms_str.encode()).hexdigest(), '5610884dc43c36ef91151e6ad36493a8')
